...
minetest.register_on_respawnplayer(function(player)
	if carregar_dados(player:get_player_name()) == false then
		...
		registros[name].destinos["Bones"] = player:getpos()
		atualizar_string(name)
		salvar_dados(name)
		minetest.chat_send_player(name, S("Lugar @1 foi gravado no seu GPS.", "Bones"))
		
		adicionar_waypoint(name, "Bones")
		minetest.chat_send_player(name, S("GPS Ativado. Destino @1 localizado.", "Bones"))
